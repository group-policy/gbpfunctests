...
class test_gbp_prs_func(object):
    ...
    def __init__(self):
        ...
        self.rule_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'rule', self.rule_name, classifier=self.cls_name, action=self.act_name)
        if self.rule_uuid == 0:
            self._log.info(
    ...
    def test_gbp_prs_func_1(self, name_uuid='', ruleset_uuid='', rep_cr=0, rep_del=0):
        ...
        if rep_del == 0 or rep_del == 1:
            ...
            if self.gbpverify.gbp_policy_verify_all(0, 'ruleset', name_uuid, ruleset_uuid) != 0:
                self._log.info(
                    "# Step 3A: Verify RuleSet is Deleted using -list option == Failed")
                return 0
            if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', name_uuid, id=ruleset_uuid, shared='False') != 0:
                self._log.info(
                    "# Step 3B: Verify RuleSet is Deleted using -show option == Failed")
                return 0
            if rep_cr == 0 and rep_del == 0:
    ...
    def test_gbp_prs_func_3(self):
        ...
        if self.gbpverify.gbp_policy_verify_all(0, 'ruleset', 'grppol_prs', ruleset_uuid) == 0:
            self._log.info(
                "# Step 2A: Verify Policy RuleSet Updated Attributes using -list option == Failed")
            return 0
        if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', ruleset_uuid, name='grppol_prs',
            ...
            self._log.info(
                "# Step 2B: Verify Policy RuleSet Updated Attributes using -show option == Failed")
            return 0
    ...
    def test_gbp_prs_func_4(self):
        ...
        ruleset_uuid = self.gbpcfg.gbp_policy_cfg_all(1, 'ruleset', 'grppol_prs_many',
                                                      policy_rule='"%s %s %s %s"' % (rule_uuid_list[0], rule_uuid_list[
                                                                                     1], rule_uuid_list[2], rule_uuid_list[3]),
        ...
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset', 'grppol_prs_many', 'policy_rules', rule_uuid_list) == 0:
            self._log.info(
                "# Step 2C: Verify Policy RuleSet and its Multiple PRs using -show option == Failed")
            return 0
        ...
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset', 'grppol_prs_many', 'policy_rules', rule_uuid_list) != 0:
            self._log.info(
                "# Step 3A: Verify Policy RuleSet and its Multiple PRs using -show option == Failed")
            return 0
        # Update and Verify the PRS by updating the PRs=NULL(unmapping all PRs)
        if self.gbpcfg.gbp_policy_cfg_all(2, 'ruleset', 'grppol_prs_many',
                                          policy_rule='""') == 0:
            self._log.info(
                "# Step 4: Upmapping All Policy Rule from Policy RuleSet , Failed")
            return 0
        if self.gbpverify.gbp_obj_ver_attr_all_values('ruleset', 'grppol_prs_many', 'policy_rules', rule_uuid_list) != 0:
            self._log.info(
                "# Step 4A: Verify All Policy Rules have been Removed from Policy RuleSet using --show option == Failed")
            return 0
    ...
    def test_gbp_prs_func_5(self):
        ...
        parent_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'ruleset', 'demo_par_prs', policy_rules=self.rule_name, child_policy_rule_sets=child_uuid)
        if parent_uuid == 0:
            self._log.info(
                "\n## Step 2: Create Parent Policy RuleSet == Failed")
            return 0
        self._log.info(
            '# Step 2A: Verify Parent and Child Policy RuleSet using -show cmd')
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', 'demo_par_prs', id=parent_uuid,
                                                ...
                                                child_policy_rule_sets=child_uuid) != 0:
            self._log.info(
                "# Step 3A: Verify Parent PRS after Delete of Child PRS using -show option == Failed")
            return 0
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2, 'ruleset', 'demo_par_prs', child_policy_rule_sets=child_uuid) == 0:
            self._log.info(
                "# Step 5: Associating Child PRS by Updating Parent PRS == Failed")
            return 0
        self._log.info(
            '# Step 5A: Verify Parent and Child Policy RuleSet using -show cmd')
        if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', 'demo_par_prs', id=parent_uuid,
                                                policy_rules=self.rule_uuid, shared='False',
                                                child_policy_rule_sets=child_uuid) == 0:
            self._log.info(
                "# Step 5A: Verify Parent RuleSet using -show option == Failed")
            return 0
        if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', 'demo_child_prs', id=child_uuid,
                                                policy_rules=self.rule_uuid, shared='False',
                                                parent_id=parent_uuid) == 0:
            self._log.info(
                "# Step 5B: Verify Parent RuleSet using -show option == Failed")
            return 0
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'ruleset', 'demo_child_prs', id=child_uuid,
                                                ...
                                                parent_id=parent_uuid) != 0:
            self._log.info(
                "# Step 6A: Verify Child PRS after Delete of Parent PRS using -show option == Failed")
            return 0
    ...
    def test_gbp_prs_func_6(self):
        ...
        for i in range(1, 3):
            ...
            if child_uuid == 0:
                ...
                return 0
            parent_uuid = self.gbpcfg.gbp_policy_cfg_all(
                1, 'ruleset', par_name, policy_rules=self.rule_name, child_policy_rule_sets=child_uuid)
            if parent_uuid == 0:
                self._log.info(
                    "\n## Step 2: Create Parent Policy RuleSet == Failed")
                return 0
