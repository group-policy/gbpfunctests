...
class test_gbp_pr_func(object):
    ...
    def test_gbp_pr_func_1(self, name_uuid='', rule_uuid='', rep_cr=0, rep_del=0):
        ...
        if rep_cr == 0 or rep_cr == 1:
            ...
            rule_uuid = self.gbpcfg.gbp_policy_cfg_all(
                1, 'rule', name_uuid, classifier=self.cls_name, action=self.act_uuid)
            if rule_uuid == 0:
        ...
        if rep_del == 0 or rep_del == 1:
            ...
            if self.gbpverify.gbp_policy_verify_all(0, 'rule', name_uuid, rule_uuid) != 0:
                self._log.info(
                    "# Step 3A: Verify Rule is Deleted using -list option == Failed")
                return 0
            if self.gbpverify.gbp_policy_verify_all(1, 'rule', name_uuid, id=rule_uuid, shared='False') != 0:
                self._log.info(
                    "# Step 3B: Verify Rule is Deleted using -show option == Failed")
                return 0
        if rep_cr == 0 and rep_del == 0:
    ...
    def test_gbp_pr_func_3(self):
        ...
        if self.gbpverify.gbp_policy_verify_all(0, 'rule', 'grppol_pr', rule_uuid, 'True') == 0:
            self._log.info(
                "# Step 2A: Verify Policy Rule Updated Attributes using -list option == Failed")
            return 0
        if self.gbpverify.gbp_policy_verify_all(1, 'rule', rule_uuid, name='grppol_pr', policy_classifier_id=new_cls_uuid,
            ...
            self._log.info(
                "# Step 2B: Verify Policy Rule Updated Attributes using -show option == Failed")
            return 0
    ...
    def test_gbp_pr_func_6(self):
        ...
        self._log.info('\n## Step 1A: Create new PA and new PC##\n')
        new_cls_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'classifier', 'grppol_pc2', protocol='tcp', port_range='100:300')
        if new_cls_uuid == 0:
            self._log.info(
                "\nNew Classifier Create Failed, hence this Testcase is ABORTED\n")
            return 0
        ...
        if new_act_uuid == 0:
            ...
            return 0
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'rule', self.rule_name, classifier=new_cls_uuid, action=new_act_uuid)
        if rule_uuid != 0:
            self._log.info(
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'classifier', new_cls_uuid, protocol='udp', direction='bi', port_range='640:1022') == 0:
            self._log.info(
                "# Step 3: Verify Policy CLassifier Updated Attributes using -show option == Failed")
            return 0
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'classifier', new_cls_uuid, protocol='tcp', direction='bi', port_range='100:300') == 0:
            self._log.info(
                "# Step 6: Verify Policy CLassifier Updated Attributes using -show option == Failed")
            return 0
