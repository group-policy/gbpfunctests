...
class test_gbp_ptg_func(object):
    ...
    def global_cfg(self):
        ...
        l3p_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'l3p', self.l3p_name, ip_pool='20.20.0.0/24', subnet_prefix_length='28')
        if l3p_uuid == 0:
            self._log.info(
    ...
    def test_gbp_ptg_func_1(self, name_uuid='', ptg_uuid='', rep_cr=0, rep_del=0):
        ...
        if rep_cr == 0 or rep_cr == 1:
            ...
            if self.gbpverify.neut_ver_all('net', ntkid, name=net_name, admin_state_up='True', subnets=subnetid) == 0:
                self._log.info(
                    "# Step 2E: Implicit-creation of Neutron Network-Obj -show option == Failed")
                return 0
    ...
    def test_gbp_ptg_func_2(self):
        ...
        uuids = self.gbpcfg.gbp_policy_cfg_all(
            1, 'group', self.ptg_name, consumed_policy_rule_sets='%s=scope' % (self.ruleset_name))
        if uuids != 0:
