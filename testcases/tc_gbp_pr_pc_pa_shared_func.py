...
class test_gbp_pr_pc_pa_shared_func(object):
    ...
    def test_gbp_pr_pc_pa_shared_func_1(self):
        ...
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(
            1, 'rule', self.rule_name, classifier=self.cls_uuid, action=self.act_uuid, shared=True)
        if rule_uuid == 0:
            self._log.info(
        ...
        # Verify PA,PC,PR
        self._log.info(
            '## Step 2: Verify Policy Rule, Policy Classifier, Policy Action with shared=True')
        for obj, uuid in obj_uuid.iteritems():
            if self.gbpverify.gbp_policy_verify_all(1, obj, uuid, shared=True) == 0:
                self._log.info(
                    "# Step 2A_%s: Verify Policy %s.upper() with shared=True, Failed" % (obj, obj))
                return 0
    ...
    def test_gbp_pr_pc_pa_shared_func_2(self):
        ...
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'pr_true', classifier=obj_uuid_true[
                                                   'classifier'], action=obj_uuid_true['action'], shared=True)
        if rule_uuid == 0:
            self._log.info(
                "# Step 3: Creation of Policy Rule with shared=True using attributes PA+PC(shared=True), Failed")
            return 0
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1, 'rule', rule_uuid, classifier=obj_uuid_false['classifier'], action=obj_uuid_false['action']) != 0:
            self._log.info(
                "# Step 4A: Updating Policy Rule(shared=True) by attributes PA+PC(shared=False) DID NOT Fail")
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'rule', 'pr_true', id=rule_uuid,
                                                policy_classifier_id=obj_uuid_true[
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1, 'rule', rule_uuid, classifier=obj_uuid_false['classifier'], action=obj_uuid_false['action'], shared=False) == 0:
            self._log.info(
                "# Step 5A: Updating Policy Rule(shared=False) by attributes PA+PC(shared=False), Failed")
            return 0
        self._log.info(
            "\n## Step 5B: Verify the Policy Rule attributes PA,PC,shared=False ##")
        if self.gbpverify.gbp_policy_verify_all(1, 'rule', 'pr_true', id=rule_uuid,
                                                policy_classifier_id=obj_uuid_false[
                                                    'classifier'], enabled='True',
    ...
    def test_gbp_pr_pc_pa_shared_func_3(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'true_pr', classifier=obj_uuid['classifier'], action=obj_uuid['action'], shared=True) != 0:
            self._log.info(
                "# Step 2A: Creation of Policy Rule with shared=True using attributes PA(shared=False)+PC(shared=True) did NOT Fail")
            return 0
        self._log.info(
            "\n## Step 2B: Create Policy Rule(shared=False) with PA(shared=False) & PC(shared=True)& Verify ##")
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'true_pr', classifier=obj_uuid[
                                                   'classifier'], action=obj_uuid['action'])
        if rule_uuid == 0:
            self._log.info(
                "# Step 2B: Creation of Policy Rule with shared=False(default) using attributes PA(shared=False)+PC(shared=True), Failed")
            return 0
        if self.gbpverify.gbp_policy_verify_all(1, 'rule', rule_uuid,
                                                policy_classifier_id=obj_uuid[
                                                    'classifier'], enabled='True',
        ...
        true_rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'true_pr', classifier=obj_uuid[
                                                        'classifier'], action=obj_uuid['action'], shared=True)
        if true_rule_uuid == 0:
            self._log.info(
                "# Step : Creation of Policy Rule with shared=True using attributes PA+PC(shared=True), Failed")
            return 0
    ...
    def test_gbp_pr_pc_pa_shared_func_4(self):
        ...
        if self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'pr_true', classifier=obj_uuid_false['classifier'], action=obj_uuid_false['action'], shared=True) != 0:
            self._log.info(
                "# Step 2B: Creation of Policy Rule with shared=True using attributes PA+PC(shared=False) DID NOT Fail")
            return 0
        ...
        rule_uuid = self.gbpcfg.gbp_policy_cfg_all(1, 'rule', 'pr_true', classifier=obj_uuid_false[
                                                   'classifier'], action=obj_uuid_false['action'], shared=True)
        if rule_uuid == 0:
            self._log.info(
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2, 'action', self.act_uuid, shared=False) != 0:
            self._log.info(
                "# Step 5A: Update of Policy Action shared=Failed DID NOT Fail")
            return 0
        self._log.info(
        ...
        if self.gbpcfg.gbp_policy_cfg_all(2, 'classifier', self.cls_uuid, shared=False) == 0:
            self._log.info(
                "# Step 5B: Update of Policy Classifier shared=False DID NOT Fail")
            return 0
        ...
        if self.gbpverify.gbp_policy_verify_all(1, 'action', self.act_uuid, shared='True') == 0:
            self._log.info(
                "# Step 6: Policy Action verify shows that shared attribute has become False")
            return 0
        if self.gbpverify.gbp_policy_verify_all(1, 'classifier', self.cls_uuid, shared='True') == 0:
            self._log.info(
                "# Step 6: Policy Classifier verify shows that shared attribute become False")
            return 0
