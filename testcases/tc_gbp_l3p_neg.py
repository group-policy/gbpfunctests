...
class test_gbp_l3p_neg(object):
    ...
    def test_gbp_l3p_neg_1(self):
        ...
        count = 0
        invalid_pools = ['x.y.0.0/24', '0.2323.0.0/24',
        ...
        for pool in invalid_pools:
            ...
            if self.gbpcfg.gbp_policy_cfg_all(1, 'l3p', self.l3p_name, ip_pool=pool) != 0:
                self._log.info(
                    "# Step 1A: Create L3Policy with Invalid IP-Pool %s did NOT fail" % (pool))
            ...
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1, 'l3p', self.l3p_name) != 0:
                self._log.info(
                    "# Step 1A: L3Policy did NOT fail to create even with Invalid IP-Pool %s" % (pool))
                count += 1
        if count > 0:
            return 0
        else:
    ...
    def test_gbp_l3p_neg_2(self):
        ...
        for prefix in invalid_prefix_length:
            ...
            if self.gbpcfg.gbp_policy_cfg_all(1, 'l3p', self.l3p_name, subnet_prefix_length=prefix) != 0:
                self._log.info(
                    "# Step 1A: Create L3Policy with Invalid IP-Pool %s did NOT fail" % (prefix))
            self._log.info('# Step 1A: Verify L3Policy did NOT get created')
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1, 'l3p', self.l3p_name) != 0:
                self._log.info(
                    "# Step 1A: L3Policy did NOT fail to create even with Invalid IP-Pool %s" % (prefix))
                cnt += 1
        if cnt > 0:
            return 0
        else:
    ...
    def test_gbp_l3p_neg_3(self):
        ...
        for ip, pref in mix_attr.iteritems():
            ...
            if self.gbpcfg.gbp_policy_cfg_all(1, 'l3p', self.l3p_name, ip_pool=ip, subnet_prefix_length=pref) != 0:
                self._log.info(
                    "# Step 1A: Create L3Policy with mix of valid and invalid did NOT fail")
            self._log.info('# Step 1A: Verify L3Policy did NOT get created')
            if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1, 'l3p', self.l3p_name) != 0:
                self._log.info(
                    "# Step 1A: L3Policy did NOT fail to create even with mix of Valid and Invalid attrs %s")
                _pass += 1
        if _pass > 0:
            return 0
        else:
    ...
    def test_gbp_l3p_neg_4(self):
        ...
        if self.gbpverify.gbp_l2l3ntk_pol_ver_all(1, 'l3p', l3p_uuid,
            ...
            return 0
        # clean-up before next testcase
        self.gbpcfg.gbp_policy_cfg_all(0, 'l3p', l3p_uuid)
